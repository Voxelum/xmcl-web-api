...
function createCloudflareApp(
  env: AppConfig,
  routeSurface: "common" | "ai" | "signaling",
) {
...
export function routeSurfaceForHost(
  ...
  if (normalized === "ai.xmcl.app") return "ai";
  if (normalized === "signaling.xmcl.app") return "signaling";
  if (normalized === "api.xmcl.app") return "common";
  if (
    configuredSurface === "ai" || configuredSurface === "signaling" ||
    configuredSurface === "common"
  ) {
...
async function dispatchCloudflareRequest(
  request: Request,
  env: any,
