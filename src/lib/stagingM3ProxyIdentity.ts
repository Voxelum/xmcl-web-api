...
export const STAGING_M3_PROXY_MAX_AGE_MS = 60_000;
export const STAGING_M3_PROXY_MAX_BODY_BYTES = 64 * 1024;

const headers = {
  keyId: "x-xmcl-staging-m3-key-id",
  timestamp: "x-xmcl-staging-m3-timestamp",
  nonce: "x-xmcl-staging-m3-nonce",
  signature: "x-xmcl-staging-m3-signature",
} as const;

export interface StagingM3ProxyNonceStore {
...
/**
 * This nonce namespace is deliberately distinct from PayPal webhook and
 * compiler workload identities. Mongo's unique `_id` makes a replay fail even
 * when two Azure instances verify it concurrently.
 */
export class MongoStagingM3ProxyNonceStore implements StagingM3ProxyNonceStore {
  ...
  async consume(input: {
    ...
    const collection = this.db.collection("staging_m3_proxy_nonces");
    const id = `staging-m3-proxy-hmac:${input.key}`;
...
export class StagingM3ProxyIdentityError extends Error {
  constructor(
    readonly code: "request_identity_rejected" | "request_replayed",
  ) {
    super(code);
    this.name = "StagingM3ProxyIdentityError";
  }
}

/**
 * Worker-to-Azure identity for authenticated M3 API calls. It intentionally
 * uses dedicated `x-xmcl-staging-m3-*` headers so the end-user Bearer
 * Authorization header is preserved for normal account authentication.
 */
export class HmacStagingM3ProxyIdentity {
  ...
  constructor(
    private readonly options: {
      keyId: string;
      secret: string;
      nonceStore?: StagingM3ProxyNonceStore;
      now?: () => number;
    },
  ) {
    ...
    if (
      ...
      throw new TypeError("invalid staging M3 proxy identity configuration");
    }
  }
  ...
  async signOutgoing(input: {
    ...
    if (!validSignedRequest({ ...input, timestamp, nonce, now: timestamp })) {
      throw new StagingM3ProxyIdentityError("request_identity_rejected");
    }
    return {
      ...
      [headers.signature]: base64Url(
        await this.signature(input.method, input.target, timestamp, nonce, input.body),
      ),
    };
  }
...
export class StagingM3ProxyBodyTooLargeError extends Error {
  constructor() {
    super("staging M3 proxy request exceeds the maximum size");
    this.name = "StagingM3ProxyBodyTooLargeError";
  }
}

export async function readStagingM3ProxyRawBody(request: Request) {
  const contentLength = request.headers.get("content-length");
  if (
    contentLength && /^[0-9]+$/.test(contentLength) &&
    Number(contentLength) > STAGING_M3_PROXY_MAX_BODY_BYTES
  ) {
    throw new StagingM3ProxyBodyTooLargeError();
  }
  ...
  try {
    while (true) {
      ...
      if (length > STAGING_M3_PROXY_MAX_BODY_BYTES) {
        await reader.cancel();
        throw new StagingM3ProxyBodyTooLargeError();
      }
      chunks.push(value);
...
function validSignedRequest(input: {
  ...
  return /^[A-Z]+$/.test(input.method) &&
    ...
    input.body.byteLength <= STAGING_M3_PROXY_MAX_BODY_BYTES &&
    Number.isSafeInteger(input.timestamp) &&
    noncePattern.test(input.nonce ?? "") && Number.isSafeInteger(input.now) &&
    Math.abs(input.now - input.timestamp) <= STAGING_M3_PROXY_MAX_AGE_MS;
}
