...
import { type S3SigV4Presigner } from "./s3SigV4.ts";
 ...
 */
export class S3SharedModdedArchiveStore implements SharedModdedArchiveStore {
  constructor(
  ...
  async createUpload(input: {
    ...
    if (
      ...
      signed.headers["x-amz-meta-sha256"] !== input.expectedSha256.toLowerCase()
    ) {
      throw new Error("exact S3 archive upload signing failed");
    }
    return {
  ...
  async readVerified(input: {
    ...
    if (signed.key !== input.key || signed.method !== "GET" || signed.headers) {
      throw new Error("exact S3 archive download signing failed");
    }
    ...
    try {
      ...
      if (
        ...
        throw new Error("exact S3 archive download failed");
      }
...
async function readExact(response: Response, expectedSize: number) {
  if (!response.body) throw new Error("S3 archive body missing");
  ...
  try {
    while (true) {
      ...
      if (total > expectedSize || total > maximumArchiveBytes) {
        throw new Error("S3 archive exceeds expected size");
      }
  ...
  if (total !== expectedSize) throw new Error("S3 archive size does not match import");
