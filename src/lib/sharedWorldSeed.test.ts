...
const archive = createStoredZip([
  {
    path: "world.json",
    bytes: jsonBytes({
      schemaVersion: 1, worldName: "Local World", source: "local_instance",
      files: [{ path: "world/level.dat", sha256: digest, sizeBytes: bytes.byteLength }],
    }),
  },
...
function fixture() {
  ...
  const scheduler = new SharedHostingScheduler(
    ...
    undefined,
    { region: "sgp", now: () => new Date(now), createId: (prefix) => `${prefix}_${++id}` },
  );
  const seeds = new SharedWorldSeedService({
    scheduler, repository: new MemorySharedWorldSeedRepository(),
    archives: {
      createUpload: async (input) => ({ uploadUrl: `https://storage.example/${input.key}`, expiresAt: now, maxSizeBytes: input.expectedSizeBytes }),
      readVerified: async () => archive,
    },
    now: () => now, createId: (prefix) => `${prefix}_${++id}`,
  });
  return { scheduler, seeds, commands };
}

async function newService(f: ReturnType<typeof fixture>) {
  return await f.scheduler.createService({ accountId: "account_1", subscriptionId: "sub_1", idempotencyKey: "service" });
}

async function completeSeed(f: ReturnType<typeof fixture>, serviceId: string, key: string) {
  const seed = await f.seeds.create({ accountId: "account_1", serviceId, expectedSha256: archiveSha, expectedSizeBytes: archive.byteLength, idempotencyKey: key });
...
Deno.test("world seeds reject cross-account access and a running service", async () => {
  ...
  await assert.rejects(
    () => f.seeds.create({ accountId: "account_other", serviceId: service.serviceId, expectedSha256: archiveSha, expectedSizeBytes: archive.byteLength, idempotencyKey: "other" }),
  ...
  await assert.rejects(
    () => f.seeds.create({ accountId: "account_1", serviceId: service.serviceId, expectedSha256: archiveSha, expectedSizeBytes: archive.byteLength, idempotencyKey: "running" }),
    /state_conflict/,
  );
});
...
Deno.test("only a validated seed becomes the initial world and failed validation preserves state", async () => {
  const f = fixture();
  const service = await newService(f);
  const bad = await f.seeds.create({ accountId: "account_1", serviceId: service.serviceId, expectedSha256: "f".repeat(64), expectedSizeBytes: archive.byteLength, idempotencyKey: "bad" });
  await f.seeds.uploadUrl("account_1", bad.seedId);
  const invalid = await f.seeds.complete("account_1", bad.seedId, "bad-complete");
  assert.equal(invalid.status, "invalid");
  assert.equal((await f.scheduler.getService("account_1", service.serviceId)).initialWorld, undefined);
...
Deno.test("a seed is selected only before first start and cannot overwrite a synced runtime world", async () => {
  ...
  await assert.rejects(
    () => f.seeds.create({ accountId: "account_1", serviceId: service.serviceId, expectedSha256: archiveSha, expectedSizeBytes: archive.byteLength, idempotencyKey: "second" }),
    /state_conflict/,
  );
  assert.equal((await f.scheduler.getService("account_1", service.serviceId)).initialWorld?.seedId, selected.seedId);
});
