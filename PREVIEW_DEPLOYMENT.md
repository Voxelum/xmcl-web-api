...
| `RTC_SECRET`, `TURNS`, `CLOUDFLARE_TURN_API_TOKEN`, `CLOUDFLARE_TURN_KEY_ID`, `CLOUDFLARE_TURN_ANALYTICS_API_TOKEN` | Preview-only RTC configuration, if RTC is required. |
...
| `XMCL_OAUTH_REDIRECT_URIS`                                         | Additional exact comma-separated HTTPS website callback URLs for previews. Production code always allows `https://xmcl.app/oauth/callback` and `https://www.xmcl.app/oauth/callback`; register both with each production provider. Launcher OAuth uses its code-owned loopback callback and needs no setting. |
